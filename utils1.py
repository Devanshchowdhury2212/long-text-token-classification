...
# Target map is different format b-lead 0 i-lead 1
target_id_map = {
    "B-Lead": 0,  "I-Lead": 1,  "B-Position": 2, "I-Position": 3, "B-Evidence": 4,
    "I-Evidence": 5, "B-Claim": 6,
    "I-Claim": 7,
...
id_target_map = {a:b for b,a in target_id_map.items()}
def _prepare_data_to_token(df,tokenizer,args,ids_array):
    training_samples = []
    for ids in tqdm(ids_array):
        with open(f"{args.file_dir}/{ids}.txt") as f:
            text = f.read()
        token = tokenizer.encode_plus(text,add_special_tokens=False,return_offsets_mapping=True)
        text_len = len(text)
        offset_mapping = token['offset_mapping']
        input_token  = token['input_ids']
        input_labels = copy.deepcopy(input_token)
        for k in range(len(input_labels)):
            input_labels[k] = 'O'
        temp = df.loc[df.id==ids]
        sample = {
            "id": ids,
            "text": text,
        ...
        for _, row in temp.iterrows():
            ...
            target_start,target_end = target_idx[0],target_idx[-1]
            label_start , label_in = 'B-'+prediction_label ,'I-'+prediction_label 
            input_labels[target_start] = label_start
